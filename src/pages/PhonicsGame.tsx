...
import GameIntro from '../componens/game/phonics/GameIntro';
...
const PhonicsGame: FC<Props> = ({ stage, step }) => {
   ...
   const onDataLoaded = useCallback(( gameData: any) => {
      ...
      const assetsList = require('../assets/PhonicsGameAssets').default( step, gameData.langCode, gameData.lowQuality );

      gameData.quizList.forEach((li: any, i: number) => {
         assetsList.push({ name: `quizAudio${i}`, url: li.audio, noRequired: true });
         assetsList.push({ name: `quizImage${i}`, url: li.image, noRequired: true });
      });
        ...
        <GameLayout 
            ...
            <Provider store={Store}>
               {(loadComplete && resources)  
                  ...
                  :
                  <Loading bgUrl='images/game/phonics/intro/bg.png' />
               }
            </Provider>
         </GameLayout>
      </div>
